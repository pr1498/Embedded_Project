...
uint32_t pattern[37][NUM_FREQ], pattern_scaled[37];
...
void loop() {
  if (tone0[0].available()) {
    ...
    if (freq_to_detect != INVALID_VAL) {
      for (int i = 0; i <= 36 ; i++) {
        if (pattern[i][freq_to_detect] >= max_freq_amp * 3 / 4) pattern_scaled[i] = 4;
        else if (pattern[i][freq_to_detect] >= max_freq_amp * 1 / 2) pattern_scaled[i] = 3;
        else if (pattern[i][freq_to_detect] >= max_freq_amp * 1 / 4) pattern_scaled[i] = 2;
