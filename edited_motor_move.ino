...
//#define PROX_TRIG_PIN_right 23//change pin
//#define PROX_ECHO_PIN_right 22//change
//#define PROX_TRIG_PIN_left 23//change
//#define PROX_ECHO_PIN_left 22//change
...
void setup() {
  ...
  // pinMode(PROX_TRIG_PIN_left, OUTPUT); // Sets the trigPin as an Output
  //pinMode(PROX_ECHO_PIN_left, INPUT); // Sets the echoPin as an Input
   //pinMode(PROX_TRIG_PIN_right, OUTPUT); // Sets the trigPin as an Output
  //pinMode(PROX_ECHO_PIN_right, INPUT); // Sets the echoPin as an Input
...
void loop() {
  ...
  if (freq_to_detect != INVALID_VAL) {
    ...
    while (finish != 1 && redo != 1 && tone_done[freq_to_detect] != 1 ) {
      ...
      freq_amp_new[freq_to_detect] = freq_amp_new[freq_to_detect]/8;
      ...
      if ( freq_amp_new[freq_to_detect] < 0.5 * max_freq_amp[freq_to_detect]) redo = 1;
